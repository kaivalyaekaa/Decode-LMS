...
const FinanceDashboard = () => {
    ...
    const fetchRegistrations = async (region) => {
        try {
            setLoading(true);
            const regsRes = await getFinanceRegistrations(region);
    ...
    const handleUpdatePayment = async () => {
        try {
            ...
            fetchRegistrations(activeTab === 'us_manual' ? REGIONS.US : '');
        } catch (error) {
